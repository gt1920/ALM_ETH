...
| Folder | Description |
|--------|-------------|
| `ALM_APP/` | Main application firmware (STM32H563, Keil MDK) |
| `ALM_Bootloader/` | IAP bootloader — decrypts and flashes `.alm` upgrades from W25Q |
| `DT_Controller_Project/` | PC control software (WinForms, .NET 4.8) |
| `Copy_Bin_Project/` | Post-build tool — encrypts `.bin` to `.alm` upgrade package |
...
|--------|---------|------|---------|
| Bootloader | `0x08000000` | 32 KB | `ALM_Bootloader` |
| Application | `0x08008000` | ~992 KB | `ALM_APP` |

W25Q16 external flash (`0x00000000`): holds the `.alm` upgrade package written by the app; cleared by bootloader after flashing.
...
Power on
  └─ Bootloader (0x08000000)
       ├─ W25Q[0x00]: no valid .alm  →  jump to App (0x08008000)
       └─ W25Q[0x00]: valid .alm found
...
PC Upgrade (DT Controller → right-click ETH device → Upgrade)
  ...
  3. CMD_UPGRADE START  — sends file_size + alm[0..15] for device-side validation
...
Generated by `Copy_Bin_Project` from the compiled `.bin`.
...
**Protocol (STM32 side — `ALM_APP/BSP/upgrade_handler.c`)**
- New command `CMD_UPGRADE (0x30)` with sub-commands START / DATA / END.
- START: receives `file_size(4) + alm[0..15](16)`, decrypts the header with AES-256-CBC, validates magic / board_id / fw_sn / CRC32 before erasing W25Q. Rejects mismatched or invalid firmware immediately.
