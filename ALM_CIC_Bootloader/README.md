...
if a `.gt` upgrade file was staged by ALM_CIC_APP; if so it decrypts, verifies
...
| `0x08008000 – 0x080FDFFF` | 984 KB | App code (ALM_CIC_APP) |
| `0x080FE000 – 0x080FFFFF` | 8 KB | App's `device_config` storage (last sector bank 2) |

ALM_CIC_APP scatter: `LR_IROM1 0x08008000 0x000F8000`.
...
| Range | Purpose |
|---|---|
| `0x000000` onwards | `.gt` file written by ALM_CIC_APP (Copy_Bin output, byte-for-byte) |
...
## Device SN Algorithm (cpu_id.c — shared with ALM_CIC_APP, byte-identical)
...
2. Set `FW_HW_ID.fw_sn = <that SN>` in [ALM_CIC_APP/BSP/fw_id.c](../ALM_CIC_APP/BSP/fw_id.c) before building.
...
│   ├── cpu_id.{c,h}        UID→SN algorithm (byte-identical to ALM_CIC_APP)
│   └── iap.{c,h}           BL_Run() — complete bootloader logic
└── MDK-ARM/
    ├── ALM_CIC_Bootloader.uvprojx   ← open in Keil
    ├── Copy_HEX.bat             auto-copies .hex to ALM_CIC_APP/HEX/app/Bootloader/
    └── ALM_CIC_Bootloader/ALM_CIC_Bootloader.sct   IROM = 32 KB @ 0x08000000
```

---

## ALM_CIC_APP Side (OTA receive — to be implemented)

When the ALM_CIC_APP receives a new `.gt` file over the network:
