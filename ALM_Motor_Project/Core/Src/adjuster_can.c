...
void CAN_Report_MotorStatus(void)
{
    ...
    txHeader.Identifier          = CAN_ID_STATUS_FEEDBACK;   // ���� 0x121
 ...
 *
 * NodeID value 0 is reserved on the CAN bus (broadcast/invalid), so a
 * non-zero result is guaranteed even in the astronomically unlikely
 * case the SPC bytes collapse to 0.
 *
...
uint32_t Build_NodeID_From_UID(void)
{
    ...
    uint8_t foldLot = (uint8_t)( ((Uid1 >>  8) & 0xFFU)
                               ^ ((Uid1 >> 16) & 0xFFU)
                               ^ ((Uid1 >> 24) & 0xFFU)
                               ^ ( Uid2        & 0xFFU)
                               ^ ((Uid2 >>  8) & 0xFFU)
    ...
    uint32_t b0 = (uint8_t)( Uid1        & 0xFFU) ^ foldLot;
    uint32_t b1 = (uint8_t)((Uid0 >> 16) & 0xFFU);
    uint32_t b2 = (uint8_t)((Uid0 >>  8) & 0xFFU);
    uint32_t b3 = (uint8_t)( Uid0        & 0xFFU);

    uint32_t id = (b0 << 24) | (b1 << 16) | (b2 << 8) | b3;

    /* CAN-bus safety: NodeID 0 is reserved for broadcast/invalid. */
    if (id == 0U)
        id |= 0x01000000UL;
