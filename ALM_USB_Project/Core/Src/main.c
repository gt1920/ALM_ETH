...
uint8_t buf[64] = {0x01, 0xAA, 0x55};
uint8_t USBD_CustomHID_OutBuffer[64];
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	filter.FilterType   = FDCAN_FILTER_RANGE;      // ÏÈÈ«ÊÕ
...
//}
		...
		//buf[3]++;
		//buf[63] = 0xFF;
		//USBD_CUSTOM_HID_SendReport(&hUsbDeviceFS, buf, 64);
    //HAL_Delay(1000);
    /* USER CODE END WHILE */
