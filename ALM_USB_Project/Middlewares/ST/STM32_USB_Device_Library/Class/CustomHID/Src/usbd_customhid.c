...
/**
  ...
  * @retval status
  */
extern uint8_t USBD_CustomHID_OutBuffer[64];
...
static uint8_t USBD_CUSTOM_HID_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
    ...
    CUSTOMHIDInEpAdd  = CUSTOM_HID_EPIN_ADDR;
    CUSTOMHIDOutEpAdd = CUSTOM_HID_EPOUT_ADDR;
    ...
    /* Open EP IN */
    USBD_LL_OpenEP(pdev, CUSTOMHIDInEpAdd, USBD_EP_TYPE_INTR, CUSTOM_HID_EPIN_SIZE);
    pdev->ep_in[CUSTOMHIDInEpAdd & 0xFU].is_used = 1U;

    /* Open EP OUT */
    USBD_LL_OpenEP(pdev, CUSTOMHIDOutEpAdd, USBD_EP_TYPE_INTR, 64);
    ...
    /* Prepare OUT endpoint receive buffer */
		USBD_LL_PrepareReceive(pdev, CUSTOMHIDOutEpAdd,
													 hhid->Report_buf,
